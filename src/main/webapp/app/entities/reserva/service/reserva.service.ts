...
type RestOf<T extends IReserva | NewReserva> = Omit<T, 'fechaHora'> & {
...
export class ReservaService {
  ...
  protected convertDateFromClient<T extends IReserva | NewReserva | PartialUpdateReserva>(reserva: T): RestOf<T> {
    return {
      ...reserva,
      fechaHora: reserva.fechaHora?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restReserva: RestReserva): IReserva {
    return {
      ...restReserva,
      fechaHora: restReserva.fechaHora ? dayjs(restReserva.fechaHora) : undefined,
    };
  }
