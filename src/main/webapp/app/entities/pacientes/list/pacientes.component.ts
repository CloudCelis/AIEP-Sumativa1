...
import { EntityArrayResponseType, PacientesService } from '../service/pacientes.service';
...
export class PacientesComponent implements OnInit {
  ...
  trackId = (item: IPacientes): number => this.pacientesService.getPacientesIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.pacientes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPacientes[] | null): IPacientes[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.pacientesService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
